...
MAC_SIG=$(gh release download "$TAG" --repo "$RELEASES_REPO" -p "*.dmg.sig" --output - 2>/dev/null || true)
WIN_SIG=$(gh release download "$TAG" --repo "$RELEASES_REPO" -p "*.msi.sig" --output - 2>/dev/null || true)

if [ -z "$MAC_SIG" ]; then
  echo "Warning: No .dmg.sig found — darwin-aarch64 will be missing from latest.json"
fi
if [ -z "$WIN_SIG" ]; then
...
# NOTE: Tauri generates filenames like: ReadCode_0.2.0_aarch64.dmg / ReadCode_0.2.0_x64_en-US.msi
# Adjust these patterns if your actual filenames differ (check the first release's bundle/ output).
MAC_ASSET=$(gh release view "$TAG" --repo "$RELEASES_REPO" --json assets \
  --jq '.assets[] | select(.name | endswith(".dmg")) | .url' 2>/dev/null || true)
WIN_ASSET=$(gh release view "$TAG" --repo "$RELEASES_REPO" --json assets \
  --jq '.assets[] | select(.name | endswith(".msi")) | .url' 2>/dev/null || true)
...
if git clone --branch gh-pages --depth 1 \
    "https://github.com/$RELEASES_REPO.git" "$WORK_DIR" 2>/dev/null; then
  echo "    Cloned existing gh-pages branch"
else
  git clone --depth 1 "https://github.com/$RELEASES_REPO.git" "$WORK_DIR"
